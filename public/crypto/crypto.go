...
func RunCrypto() {
	...
	var (
		originFilePath  = "doc/env.yaml" // 加密原文件
		encryptFilePath = "doc/env.bin"  // 加密后文件
		prefixKey       = "Er0#8&U9"     // AES-128, AES-192, AES-256：16, 24, 32 bytes
	)

	// 获取环境配置加密变量
	suffixKey, ok := os.LookupEnv("KEY")
	if !ok {
		log.Fatalf("获取加密配置环境变量参数KEY失败")
	}

	// 配置加密
	err := encryptConfigFile(originFilePath, encryptFilePath, prefixKey+suffixKey) // 输出 env.bin 加密文件
	if err != nil {
		log.Fatalf("配置文件加密失败")
	}

	fmt.Println("配置文件加密成功，已生成加密文件：" + encryptFilePath)
